...
${PSEUDO}chown -R ${USER:-alpine}:${USER:-alpine} ${LOG_PATH}
...
# alpine is not supported by PCP
...
if [[ -z "${ENTRYPOINT_WORKDIR}" ]]; then
    if [[ $EUID -ne 0 ]]; then ENTRYPOINT_WORKDIR="/home/${USER:-alpine}"; else ENTRYPOINT_WORKDIR="/root"; fi
fi
